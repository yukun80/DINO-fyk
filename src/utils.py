...
def load_model(model_type, data_dir):
    if model_type == "robust_resnet50":
        ...
        if not os.path.exists(model_file):
            wget.download("http://6.869.csail.mit.edu/fa19/psets19/pset6/imagenet_l2_3_0.pt", model_file)
        model_weights = torch.load(model_file)
        model_weights_modified = {
            name.split("model.")[1]: value for name, value in model_weights["model"].items() if "model" in name
        }
...
def get_transform(res, is_label, crop_type):
    ...
    if is_label:
        ...
        return T.Compose([T.Resize(res, Image.NEAREST), cropper, T.ToTensor(), normalize])
...
class UnsupervisedMetrics(Metric):
    def __init__(
        self, prefix: str, n_classes: int, extra_clusters: int, compute_hungarian: bool, dist_sync_on_step=True
    ):
    ...
    def compute(self):
        if self.compute_hungarian:
            ...
            self.assignments = (torch.arange(self.n_classes).unsqueeze(1), torch.arange(self.n_classes).unsqueeze(1))
        ...
        metric_dict = {self.prefix + "mIoU": iou[~torch.isnan(iou)].mean().item(), self.prefix + "Accuracy": opc.item()}
